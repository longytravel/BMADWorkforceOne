...
- [ ] Create HeatmapDetailModal component (AC: 1, 2, 6, 7)
  - [ ] Create src/components/CvDHeatmap/HeatmapDetailModal.tsx
  - [ ] Use Shadcn Dialog component as base
  - [ ] Accept dataPoint prop (CvDDataPoint type)
  - [ ] Accept isOpen and onClose props for modal control
  - [ ] Implement header with time interval display
  - [ ] Implement close button (X icon)
  - [ ] Support Escape key to close (Dialog handles this)
  - [ ] Support click-outside to close (Dialog handles this)

- [ ] Implement coverage summary section (AC: 2)
  - [ ] Display coverage percentage with color-coded badge (green/yellow/red)
  - [ ] Display scheduled agents count
  - [ ] Display forecasted demand (calls)
  - [ ] Use Card or similar Shadcn component for layout

- [ ] Implement scheduled agents list (AC: 3, 4, 8)
  - [ ] Query Zustand store for agents scheduled in this interval
  - [ ] Filter activities that overlap with intervalStart time
  - [ ] Display agent name and activity type
  - [ ] Show activity type badge with color coding (shift=blue, break=orange, etc.)
  - [ ] Make list scrollable if > 10 agents (max-height: 240px with overflow-y-auto)
  - [ ] If > 20 agents, implement "Show more" button or pagination (recommended)
  - [ ] Handle edge case: No agents scheduled (display "No agents scheduled")

- [ ] Implement skills breakdown section (AC: 5)
  - [ ] Extract skills from scheduled agents
  - [ ] Aggregate skill counts (e.g., { Spanish: 4, Technical: 3 })
  - [ ] Display skills with counts in badge format
  - [ ] Sort skills by count (descending)
  - [ ] Handle edge case: No skills (display "No skills data")

- [ ] Integrate modal with HeatmapCell (AC: 1, 6)
  - [ ] Update HeatmapCell.tsx to support onClick handler
  - [ ] Add state management for selected cell (useState or lift to CvDHeatmap)
  - [ ] Pass selected dataPoint to HeatmapDetailModal
  - [ ] Open modal when cell is clicked
  - [ ] Close modal when onClose callback is triggered

- [ ] Style modal for professional appearance
  - [ ] Use Shadcn Dialog styling (white background, shadow, rounded corners)
  - [ ] Add section headers (h3 for "Coverage Summary", "Scheduled Agents", "Skills Breakdown")
  - [ ] Apply Tailwind spacing and typography classes
  - [ ] Ensure modal is centered on screen
  - [ ] Add smooth animation (fade-in, scale)

- [ ] Write component tests (AC: 1, 2, 3, 5, 6, 7)
  - [ ] Create src/components/CvDHeatmap/HeatmapDetailModal.test.tsx
  - [ ] Test modal opens when isOpen=true
  - [ ] Test modal displays correct time interval
  - [ ] Test modal displays coverage summary
  - [ ] Test modal displays agent list
  - [ ] Test modal displays skills breakdown
  - [ ] Test modal closes on Escape key
  - [ ] Test modal closes on click outside
